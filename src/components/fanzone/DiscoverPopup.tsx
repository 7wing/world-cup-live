...
export function DiscoverPopup({ onClose }: { onClose: () => void }): JSX.Element {
  ...
  const { mutate: sendRequest } = useSendFriendRequest(user?.id ?? '')
  ...
  return (
    <div
      className="fixed inset-0 z-[200] bg-black/70 backdrop-blur-[10px] flex items-start justify-center pt-16 pb-4 px-4"
      onClick={onClose}
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className="w-full max-w-[720px] glass-card rounded-2xl flex flex-col max-h-[85vh]"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/5">
          <h2 className="font-lexend font-black text-xl uppercase italic text-white">
            Discover Players
          </h2>
          <button
            onClick={onClose}
                      ...
                      {user && player.id !== user.id && (
                        <NeonButton
                          ...
                          onClick={() => {
                            ...
                            sendRequest({ userId: user.id, friendId: player.id }, {
